...
<head>
  ...
  <script>
  'use strict'
  	 async function diaryDelete(){
	  		let idx = document.getElementById("idx").value;
	  		//let query = {idx : idx};
	  		
	    	const res = await fetch("http://192.168.50.60:9090/javaProject3/myDiaryDeleteProc.diary?idx="+idx,{
	    		method: "Get",
	    		headers: {
	    	...
	    	.then((data) => {
	    		if(data=='1'){
	    			alert("삭제되었습니다.");
	    			location.href="adminDiaryList.diary"
	    		}
	    		else
	    			alert("삭제실패");
	    	});
...
<div class="container">
  ...
  <input type="button" value="답글쓰기" onclick="location.href='myDiaryRewrite.diary?ref=${vo.ref}&re_step=${vo.re_step}&re_level=${vo.re_level}'" class="btn btn-warning"/>
  <input type="button" value="돌아가기" onclick="location.href='myDiaryList.diary?pag=${pag}&pageSize=${pageSize}'" class="btn btn-warning"/>
  <input type="button" value="수정하기" onclick="location.href='myDiaryUpdate.diary?idx=${vo.idx}&pag=${pag}&pageSize=${pageSize}'" class="btn btn-primary"/> &nbsp;
 	<input type="button" value="삭제하기" onclick="diaryDelete()" class="btn btn-primary"/>
</div>
<p><br/></p>
