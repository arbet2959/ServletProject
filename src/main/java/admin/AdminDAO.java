...
//댓글도 최신순서로 정렬
	 ...
	 *  WHERE  ref = (원글의 ref)
		...
		public ArrayList<DiaryReplyVO> getReplyList(int diary_Idx, int startIndexNo, int replyPageSize) {
			...
			try {
				sql = "select * from diaryReply where diary_Idx=? order by ref desc, re_level asc limit ?,?;";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, diary_Idx);
				pstmt.setInt(2, startIndexNo);
		...
		public ArrayList<DiaryVO> getOpenDiaryList(int startIndexNo, int pageSize) {
			ArrayList<DiaryVO> vos = new ArrayList<DiaryVO>();
			try {
				sql = "select B.* "
						+ "from (select idx,ref from diary where openLevel=0 and re_step=1) as A left outer join diary as B on A.ref = B.ref "
						+ "order by B.ref desc, B.re_level desc limit ?,?";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, startIndexNo);
				pstmt.setInt(2, pageSize);
		...
		public int setDiaryComplaintInput(DiaryComplaintVO vo) {
			
			try {
				sql = "insert into complaint value(default,?,?,?)";
				pstmt = conn.prepareStatement(sql);
				pstmt.setInt(1, vo.getDiary_Idx());
				pstmt.setString(2, vo.getCpMid());
		...
		public ArrayList<DiaryComplaintVO> getComplaintList() {
			ArrayList<DiaryComplaintVO> vos = new ArrayList<DiaryComplaintVO>();
			try {
				sql="select A.cpMid as cpMid, A.cpContent as cpContent, B.nickName as name, B.title as title, "
						+ "A.idx as idx, A.diary_Idx as diary_Idx "
						+ "from complaint as A inner join diary as B on A.diary_Idx = B.idx";
				pstmt= conn.prepareStatement(sql);
				rs = pstmt.executeQuery();
				while(rs.next()) {
					DiaryComplaintVO vo = new DiaryComplaintVO();
					vo.setCpMid(rs.getString(1));
					vo.setCpContent(rs.getString(2));
					vo.setName(rs.getString(3));
					vo.setTitle(rs.getString(4));
					vo.setIdx(rs.getInt(5));
					vo.setDiary_Idx(rs.getInt(6));
					vos.add(vo);
				}
		...
		public int deleteComplaint(int idx) {
			try {
